...
function completePurchase() {
    const cart = getCookie('cart');
    const productDetails = getCookie('productDetails');
    const productQuantities = cart ? JSON.parse(cart) : [];
    const products = productDetails ? JSON.parse(productDetails) : [];

    console.log('Product details:', products); // Debugging

    const amountInput = document.getElementById('amount');
    const amount = parseFloat(amountInput.value);
    const total = parseFloat(document.getElementById('total').textContent.replace('Total: €', ''));

    if (isNaN(amount) || amount < total) {
        alert('Insufficient amount. Please enter a valid amount.');
        return;
    }
    ...
    productQuantities.forEach(item => {
        ...
        if (product) {
            ...
            fetch(`http://localhost:8081/api/product/update/${product.id}`, {
                method: 'PUT',
                headers: {
    ...
    setCookie('cart', JSON.stringify([]), 7); // Clear the cart
    setCookie('productDetails', JSON.stringify([]), 7); // Clear the product details
    updateCartCount();
    updatePaymentSummary();
}